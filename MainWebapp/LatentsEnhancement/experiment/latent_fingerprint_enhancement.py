...
class TestNetwork(object):
    ...
    def _check_resume(self):
        ...
        if resume_path is not None:
            ...
            print(f"Looking for resume model at: {absolute_resume_path}")
            if os.path.isfile(absolute_resume_path):
                resume_model = torch.load(absolute_resume_path, map_location=self.device)
                self.FingerGAN_model.load_state_dict(resume_model['UNet_model_state'])
            else:
                print(f"No resume_model found at '{absolute_resume_path}'")
                sys.exit(1)
    ...
    def latentEnhance(self, latent):
        ...
        if ROW < self.input_row or COL < self.input_col:
...
def enhance():
    ...
    if file:
        ...
        img = img.astype(np.float32) / 255.0
        img = torch.from_numpy(img)
        en_latent = train_network.latentEnhance(img)
        if len(en_latent) == 0:
            return jsonify({'error': 'Enhancement failed'}), 400
        en_latent = en_latent.cpu()
        out_img = en_latent.numpy()
        out_file_path = os.path.join('enhanced', file.filename)
        cv2.imwrite(out_file_path, out_img*255)
        return jsonify({'enhanced_image': out_file_path}), 200
    else:
